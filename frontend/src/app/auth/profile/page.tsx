...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    profile_picture: authUser?.person?.profile_picture || undefined,
  });
  ...
  const [profilePicture, setProfilePicture] = useState<string>(
    useSelectedModeStore.getState().profile_picture
      ? URL.createObjectURL(
          useSelectedModeStore.getState().profile_picture as File
        )
      : typeof pic === "string"
      ? pic
      : pic.src
  ...
  // Fetch profile image if not in store
  React.useEffect(() => {
    if (!authUser) return;
    if (useSelectedModeStore.getState().profile_picture) return;
  ...
  // Re-assign all form fields from authUser if it changes
  React.useEffect(() => {
    if (!authUser) return;
    setForm({
      email: authUser.email || "",
  ...
  const handleResizeConfirm = async () => {
    ...
    if (blob) {
      const resizedFile = new File([blob], rawImage?.name || "profile.png", {
        type: "image/png",
      });
  ...
  const handleResizeCancel = () => {
    ...
    setRawImage(null);
    setPreviewUrl(
      form.profile_picture ? URL.createObjectURL(form.profile_picture) : null
    );
    setZoom(1);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.profile_picture instanceof File) {
      ...
      // If no image selected, convert profilePicture url to File and append
      if (profilePicture && profilePicture.startsWith("blob:")) {
        // Already a blob url, try to fetch and convert to File
        try {
    ...
    if (res) {
      useSelectedModeStore.setState({ profile_picture: form.profile_picture ?? undefined });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-gradient-to-br from-stone-400 via-stone-100 to-stone-300 py-20">
      {/* Role Modal */}
      {isRoleModalOpen && (
