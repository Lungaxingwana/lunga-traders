...
export default function ProductItem({ product, setUnauthorizedModalOpen }: ProductItemProps) {
  ...
  useEffect(() => {
    if (
      ...
      const fetchImages = async () => {
        ...
        // Prevent future fetches by mutating product.appearance.images_src directly
        product.appearance.images_src = images;
      };
      fetchImages();
    } else {
      // Only run this if images_src is still not set after fetch
      if (
        !product.appearance.images_src ||
        product.appearance.images_src.length === 0
      ) {
      ...
      <div className="w-full items-center justify-center relative pb-2 flex-wrap align-middle">
        ...
        <button
          onClick={() => {
            ...
            useProductStore.setState({
              allProducts: (useProductStore.getState().allProducts ?? []).map(
                (p) =>
                  p._id === product._id
                    ? {
                        ...p,
            ...
            {isLoading ? (
              ...
              >
                {lstImages.map((image, index) => {
                  // Ensure a unique key for each image, fallback to index+product._id+user_id if name is missing/empty
                  let key = `img-${index}-${product._id}-${product.general?.user_id ?? "nouser"}`;
                  if (
                    image &&
                    typeof image === "object" &&
                    "name" in image &&
                    (image as File).name &&
                    (image as File).name.trim() !== ""
                  ) {
                    key = `${(image as File).name}-${index}-${product._id}-${product.general?.user_id ?? "nouser"}`;
                  }
                  return (
                    <SwiperSlide key={key}>
                      <div className="w-full h-64 rounded-xl overflow-hidden relative flex">
                        <img
