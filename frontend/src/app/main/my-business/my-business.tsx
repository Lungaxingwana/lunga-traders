...
import ProductItemBu from "./ProductItem";
export default function MyBusinessScreen() {
  ...
  const filteredTickets = useMemo(() => {
    return (
      allProducts?.filter(t=>t.general.user_id === authUser?._id).filter((pro) => {
  ...
  return (
    <>
      <title>Lunga Traders | My Business</title>
      <AnimatePresence mode="wait">
        ...
        >
          <motion.div className="max-w-7xl w-full flex flex-col items-center justify-center mt-5 pb-20">
            ...
            <div className="w-full mx-auto px-4 items-center justify-center flex flex-col mb-5">
              <button
                onClick={() => navigate("/add-product")}
                className="p-2 bg-gradient-to-r from-stone-800 via-stone-300 to-stone-800 rounded-full hover:opacity-70 active:opacity-40 cursor-pointer w-80 shadow-lg shadow-black flex items-center justify-center gap-2"
              >
                <FaPlus size={16} />
                Add Product
              </button>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {paginatedTickets?.map((product) => (
                    <ProductItemBu key={product._id} product={product}/> // Assuming you have a ProductItem component
                  ))}
                </motion.div>
              </AnimatePresence>
            )}
