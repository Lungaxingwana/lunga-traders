...
export default function ProductDetail() {
  const { isUpdatingProduct, allProducts, updateProduct } = useProductStore();
  ...
  useEffect(() => {
    ...
    if (prod?.appearance?.images_src && prod.appearance.images_src.length > 0) {
      // Images already loaded, no action needed
    } else if (
  ...
  async function handleUpdateProduct() {
    ...
    const res = await updateProduct(prod._id, formData);
    if(res) {
