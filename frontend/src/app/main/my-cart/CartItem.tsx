  ...
  useEffect(() => {
    ...
    if (product?.appearance?.images_src?.[0]) {
      if (typeof product.appearance.images_src[0] === "string") {
        // Use a unique key for the image if possible
        setDisplayImage(product.appearance.images_src[0]);
      } else {
        // Use File.name as part of the key if available
        const file = product.appearance.images_src[0];
      ...
      fetch(
        ...
        .then(async (res) => {
          if (res.ok) {
            ...
            // Use id as part of the key for uniqueness
